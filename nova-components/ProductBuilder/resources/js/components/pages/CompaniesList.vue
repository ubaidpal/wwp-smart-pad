            ...
            <div class="flex items-center mb-6">
                <div class="flex-no-shrink ml-auto">
                    <router-link :to="{
                        ...
                        }" class="btn btn-default btn-primary">Create Company</router-link>
                </div>
            </div></div>
                    ...
                    >
                        <template slot="table-row" slot-scope="props">
                            ...
                            <span v-else-if="props.column.field == 'action'">
                                <span>
                                    <router-link :to="{name: 'edit',params: {resourceName:'companies',resourceId:props.row.id}}" class="cursor-pointer text-70 hover:text-primary mr-3 spp-save" >
    ...
    export default {
        ...
        methods: {
            ...
            confirmDelete() {
                ...
                Nova.request().post('/nova-vendor/ProductBuilder/destroyCompany',formData);
            ...
            getCompanies(){
                Nova.request().get(`/nova-vendor/ProductBuilder/companies`).then((response) => {
                    this.rows = response.data.companies;
                });
            }

        },
        async created(){
            await axios.get(`/nova-vendor/ProductBuilder/companies`).then((response) => {
                this.rows = response.data.companies;
            });

            if(!this.resourceName){
                this.resourceName = 'companies';
            }
            this.breadcrumbs=[
