        ...
        <div class="flex" style="">
            ...
            <div class="flex-no-shrink ml-auto">
                <router-link :to="{
                        ...
                        }" class="btn btn-default btn-primary">Create Role</router-link>
            </div>
        </div></div>
                    ...
                    >
                        <template slot="table-row" slot-scope="props">
                            <span v-if="props.column.field == 'permissions'">
                                <div class="flex" v-if="props.row.id > 1">
                                    <ul class="list-reset items-top w-full">
                                        <li class="flex items-center mb-2" v-for="(p,key) in props.row.permissions">
                                            <span class="text-success inline-block mr-4">
                                                <svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 28 28" class="block fill-current h-4 w-4">
                                                    <title>check</title>
                                ...
                                <div v-else>Not Limited</div>
                            </span>
                            <span v-else-if="props.column.field == 'action'">
                                <span v-if="props.row.id > 2">
                                    <router-link title="Edit Role" :to="{name: 'edit',params: {resourceName:'roles',resourceId:props.row.id}}" class="cursor-pointer text-70 hover:text-primary mr-3 spp-save" >
...
<script>
  ...
  export default {
        ...
        data: () => ({
            ...
            breadcrumbs:[],
            permissionsData:[
                {name:"users_country", label:"Country Administrator"},
                {name:"products", label:"Products"},
                {name:"product_collection", label:"Product Collections"},
                {name:"product_not_in", label:"Product Not In Collection"},
                {name:"items", label:"Items Manager"},
                {name:"dealers", label:"Dealers Manager"},
        ...
        methods: {
            ...
            getPermissionLabel(pname){
                let name = '';
                this.permissionsData.forEach((option)=>{
            ...
            confirmDelete() {
                ...
                Nova.request().post('/nova-vendor/ProductBuilder/destroyRole',formData);
            ...
            getRoles(){
                Nova.request().get(`/nova-vendor/ProductBuilder/getRoles`).then((response) => {
      ...
      async created(){
          await axios.get(`/nova-vendor/ProductBuilder/getRoles`).then((response) => {
              this.rows = response.data.roles;
          });
