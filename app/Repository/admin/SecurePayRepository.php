<?php
/**
 * Created by   :  Nabeel Qadri
 * Project Name : wwp2
 * Product Name : PhpStorm
 * Date         : 6/18/19 2:19 PM
 * File Name    : SecurepayRepository.php
 */
...
class SecurePayRepository extends PaymentGateway
{
    ...
    function __construct($AccountName = null, $AccountPassword = null, $TestMode = FALSE) {
        if ($AccountName && $AccountPassword)
            $this->setAuth($AccountName, $AccountPassword, $TestMode);
     ...
     * @return void
     */
    function setAuth($AccountName, $AccountPassword, $TestMode = FALSE) {
     ...
     */
    function processPayment($ChargeAmount = null, $params = []) {
        $rules = [
        ...
        if($validated) {
            ...
            if((FALSE === $this->ResponseXml) || (FALSE == is_string($this->ResponseXml))) {
                return [
                    'success' => false,
                    'status' =>$this->_TranslateServerCode(512)
                ];
            }
